...
public class SmoothProgressDrawable extends Drawable implements Animatable {
    ...
    private SmoothProgressDrawable(Interpolator interpolator, int sectionsCount,
                                   int separatorLength, int[] colors, float strokeWidth,
                                   float speed, float progressiveStartSpeed,
                                   float progressiveStopSpeed, boolean reversed,
                                   boolean mirrorMode, Callbacks callbacks,
    ...
    private void drawGradient(Canvas canvas) {
        ...
        float left = mReversed ? (mMirrorMode ? Math.abs(mBounds.left - mBounds.right) / 2
                : mBounds.left) : mBounds.left;
        float right = mMirrorMode ? (mReversed ? mBounds.left : Math.abs(mBounds.left
                - mBounds.right) / 2) : mBounds.right;
    ...
    private void drawStrokes(Canvas canvas) {
        ...
        for (int i = 0; i <= mCurrentSections; ++i) {
            ...
            if (end > prevValue && i >= mStartSection) {
                float xFinishingOffset = mInterpolator.getInterpolation(Math.min(mFinishingOffset,
                        1f));
    ...
    public static class Builder {
        ...
        public SmoothProgressDrawable build() {
            if (mGenerateBackgroundUsingColors) {
                mBackgroundDrawableWhenHidden = generateDrawableWithColors(mColors, mStrokeWidth);
            }
